...
//   }
  ...
  return (
    ...
    <tbody>
        {
            maintenance.filter(m=>m.ownerFirstName===sessionStorage.getItem("firstName")).map((item,i)=>{
                return(
                    <tr>
                        <th scope="row">{i+1}</th>
                        ...
                        <td>{item.others}</td>
                        <td>{item.total}</td>
                        {/* <td><button type="button" class="btn btn-danger" onClick={()=>deleteMaintenance(item.id)}>Delete</button></td> */}
                    </tr>
                )
            })
        }
